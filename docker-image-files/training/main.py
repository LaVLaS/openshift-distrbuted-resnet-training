...
def get_cgroup_metrics():
    ...
    with open(cpu_usage_path, "r") as f:
        line = f.readline()
        while line:
            if line.startswith(cpu_usage_stat_name):
                cpu_usage = (
                    int(line.removeprefix("usage_usec").strip()) / 1e9
...
def train(model, train_loader, optimizer, criterion, device, epoch, args):
    ...
    max_samples = (
        args.max_samples if args.max_samples != -1 else dataset_size
    )  # Set max_samples based on the argument or dataset size
    ...
    for batch_idx, (inputs, labels) in enumerate(train_loader):
        ...
        if batch_idx % args.print_interval == 0:
            ...
            accuracy = 100.0 * total_correct / total_samples  # Calculate accuracy
            gpu_utilization, gpu_memory_usage = get_gpu_utilization_and_memory(
                0
            )  # Get GPU metrics (assuming single GPU)
            cpu_usage, memory_usage = get_cgroup_metrics()  # Get CPU and memory metrics

            if batch_idx == 0:
                # Print CSV header
                print(
                    "epoch,batch_idx,num_samples,percentage_completed,loss,time_elapsed,learning_rate,accuracy,gpu_utilization,gpu_memory_usage,cpu_usage,mem_usage"
                )

            # Print CSV formatted metrics
            percentage_completed = (total_samples / max_samples) * 100
            print(
                f'{epoch},{batch_idx},{total_samples},{percentage_completed:.2f}%,{loss.item():.6f},{elapsed_time:.2f},{optimizer.param_groups[0]["lr"]:.6f},{accuracy:.2f},{gpu_utilization},{gpu_memory_usage},{cpu_usage},{memory_usage}'
            )
...
def main():
    ...
    parser.add_argument(
        "--pretrained_weights", type=str, default="", help="Path to pre-trained weights"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Maximum number of samples per epoch (-1 for full dataset, only works with a single node)",
    )
    parser.add_argument(
        "--print_interval",
        type=int,
        default=10,
        help="Interval for printing metrics (in batches)",
    )
    parser.add_argument("--use_syn", action="store_true", help="Use synthetic data")
    parser.add_argument(
        "--output_dir", type=str, default=".", help="Output directory for saving models"
    )
    ...
    transform = transforms.Compose(
        [
            ...
            ),  # default values, see  https://pytorch.org/vision/main/generated/torchvision.transforms.Normalize.html
        ]
    )

    trainset = datasets.ImageFolder(
        root=os.path.join(args.data_path, "train"), transform=transform
    )
    model = resnest50_fast_1s1x64d(pretrained=False)  # Loading ResNeSt model

    sampler = (
        DistributedSampler(trainset) if dist.is_initialized() else None
    ...
    if args.lr_scheduler_type == "cosine":
        scheduler = CosineAnnealingLR(
            optimizer, T_max=args.num_train_epochs
        )  # Learning rate scheduler

    for epoch in range(args.num_train_epochs):
        train(
            model, train_loader, optimizer, criterion, torch.device("cuda"), epoch, args
        )  # Training function
